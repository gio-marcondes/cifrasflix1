...
function criarAudioSilencioso(duracao = 60, sampleRate = 44100) {
    const numCanais = 1;
    const numSamples = Math.floor(duracao * sampleRate);
...
function snapTime(time, bpm, snapSubdivision, duration) {
    if (!snapSubdivision || snapSubdivision === 'off') return time;
    const beatDuration = 60 / bpm;
    let factor = 1;
    if (snapSubdivision === '1/4') factor = 0.25;
    else if (snapSubdivision === '1/2') factor = 0.5;
    else if (snapSubdivision === '1/1') factor = 1.0;
    
    const snapUnit = beatDuration * factor;
    const snapped = Math.round(time / snapUnit) * snapUnit;
    return Math.max(0, Math.min(duration, snapped));
}

function atualizarRackFxStem(content, track) {
    // Evita ReferenceError no slider de EQ do FX modal
}
...
async function ensureStemAudioContext() {
    ...
    if (!masterStemState.masterGain) {
        ...
        // Inicializa o canal FX do Master
        const ctx = masterStemState.context;
        const fxState = criarEstadoFxStem();
        fxState.eq.enabled = false;
        const fxNodes = criarFxNodesStem(ctx, fxState);
        const eqFilters = masterEqFreqs.map((freq) => {
        ...
        // Encadeamento do EQ de 10 bandas do Master
        eqFilters[0].connect(eqFilters[1]);
        for (let i = 1; i < eqFilters.length - 1; i++) {
            eqFilters[i].connect(eqFilters[i+1]);
        }
        eqFilters[eqFilters.length - 1].connect(fxNodes.input);

        // Saída do Master FX conectado ao Analisador/Destination
        const destination = masterEqState.analyser || ctx.destination;
        fxNodes.output.connect(destination);

        // Conecta masterGain ao início da cadeia de efeitos do Master
        masterStemState.masterGain.connect(eqFilters[0]);

        masterStemState.masterTrack = {
            row: { dataset: { title: 'Mix Geral (Master)' } },
            fxState,
            fxNodes,
            eqFilters,
    ...
    row.innerHTML = `
                <div class="stem-meta">
                    <div class="stem-name-container" style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                        <p class="stem-name" style="margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1; cursor: pointer;" title="Clique duas vezes para renomear"></p>
                        <div class="stem-actions" style="display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
                            ...
                            <button class="stem-duplicate-btn" type="button" title="Duplicar esta stem" style="background: transparent; border: none; cursor: pointer; padding: 2px; font-size: 16px; color: var(--accent, #39d7e6); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.15)'" onmouseout="this.style.transform='scale(1)'">
                                👥
                            </button>
                            <button class="stem-delete-btn" type="button" title="Excluir esta stem" style="background: transparent; border: none; cursor: pointer; padding: 2px; font-size: 16px; color: var(--accent, #39d7e6); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.15)'" onmouseout="this.style.transform='scale(1)'">
                                🗑️
                            </button>
                        </div>
                    </div>
...
function desenharPicosStem(canvas, picos) {
    ...
    // Desenhar linhas do grid baseadas no BPM e snap se o grid estiver ativo
    try {
        const snapSub = document.getElementById('stemSnapSelect')?.value || 'off';
        const bpm = Number(document.getElementById('stemBpmInput')?.value || 120);
        const row = canvas.closest('.stem-row');
        const duration = row ? Number(row.dataset.duration || 0) : 0;

        if (snapSub !== 'off' && bpm > 0 && duration > 0) {
            const beatDuration = 60 / bpm;
            let factor = 1;
            if (snapSub === '1/4') factor = 0.25;
            else if (snapSub === '1/2') factor = 0.5;
            else if (snapSub === '1/1') factor = 1.0;

            const gridInterval = beatDuration * factor;
            ctx.strokeStyle = 'rgba(255, 255, 255, 0.12)';
            ctx.lineWidth = 1;
            for (let t = gridInterval; t < duration; t += gridInterval) {
                const x = (t / duration) * largura;
                ctx.beginPath();
                ctx.moveTo(x, 0);
                ctx.lineTo(x, altura);
                ctx.stroke();
...
function conectarEventosStem(row, track) {
    ...
    const duplicateBtn = row.querySelector('.stem-duplicate-btn');
    const deleteBtn = row.querySelector('.stem-delete-btn');
    const stemNameEl = row.querySelector('.stem-name');
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', (event) => {
            event.stopPropagation();
            if (confirm(`Tem certeza que deseja excluir a stem "${track.row.dataset.title}"?`)) {
                removerStem(track);
            }
        });
    }
    ...
    if (stemNameEl) {
        stemNameEl.addEventListener('dblclick', (event) => {
            event.stopPropagation();
            const novoNome = prompt("Digite o novo nome para a faixa:", track.row.dataset.title);
            if (novoNome && novoNome.trim() !== "") {
                track.row.dataset.title = novoNome.trim();
                stemNameEl.textContent = novoNome.trim();
            }
        });
    }
    ...
    if (waveHit) {
        ...
        const dragHandle = (handle, side) => {
            ...
            handle.addEventListener('pointerdown', (e) => {
                ...
                const onMove = (ev) => {
                    ...
                    const snapSub = document.getElementById('stemSnapSelect')?.value || 'off';
                    const bpm = Number(document.getElementById('stemBpmInput')?.value || 120);
                    if (snapSub !== 'off' && stemTotal > 0) {
                        const snappedTime = snapTime(pct * stemTotal, bpm, snapSub, stemTotal);
                        pct = snappedTime / stemTotal;
                    }
        ...
        if (wrap) {
            ...
            wrap.addEventListener('pointerdown', (e) => {
                ...
                const rect = wrap.getBoundingClientRect();
                let initialX = (e.clientX - rect.left) / rect.width;
                initialX = Math.max(0, Math.min(1, initialX));

                const stemTotal = obterDuracaoStem(track);
                const snapSub = document.getElementById('stemSnapSelect')?.value || 'off';
                const bpm = Number(document.getElementById('stemBpmInput')?.value || 120);
                if (snapSub !== 'off' && stemTotal > 0) {
                    const snappedTime = snapTime(initialX * stemTotal, bpm, snapSub, stemTotal);
                    startX = snappedTime / stemTotal;
                } else {
                    startX = initialX;
                }
                ...
                const updateSelection = (ev) => {
                    ...
                    const stemTotal2 = obterDuracaoStem(track);
                    const snapSub2 = document.getElementById('stemSnapSelect')?.value || 'off';
                    const bpm2 = Number(document.getElementById('stemBpmInput')?.value || 120);
                    if (snapSub2 !== 'off' && stemTotal2 > 0) {
                        const snappedTime = snapTime(currentX * stemTotal2, bpm2, snapSub2, stemTotal2);
                        currentX = snappedTime / stemTotal2;
                    }
...
async function duplicarStem(track) {
    if (!track) return;

    const originalTitle = track.row.dataset.title || 'Stem';
    const newFaixa = {
        titulo: originalTitle.endsWith(' (Cópia)') ? originalTitle : `${originalTitle} (Cópia)`,
        nome: track.row.dataset.file || '',
        url: track.row.dataset.src || '',
    };

    const newTrack = await adicionarFaixaStem(newFaixa);
    if (!newTrack) return;

    if (newTrack.volume && track.volume) {
        newTrack.volume.value = track.volume.value;
        newTrack.volume.dispatchEvent(new Event('input'));
    }

    newTrack.panValue = track.panValue;
    const panKnob = newTrack.row.querySelector('.stem-pan-knob');
    const panOut = newTrack.row.querySelector('.stem-pan-value');
    if (panKnob) {
        panKnob.style.setProperty('--angle', `${(newTrack.panValue / 100) * 135}deg`);
        panKnob.setAttribute('aria-valuenow', `${Math.round(newTrack.panValue)}`);
    }
    if (panOut) panOut.textContent = formatPanStem(newTrack.panValue);

    if (track.row.classList.contains('is-muted')) {
        newTrack.row.classList.add('is-muted');
        const muteBtn = newTrack.row.querySelector('.stem-mute');
        if (muteBtn) muteBtn.classList.add('is-active');
    }

    if (track.row.classList.contains('is-solo')) {
        newTrack.row.classList.add('is-solo');
        const soloBtn = newTrack.row.querySelector('.stem-solo');
        if (soloBtn) soloBtn.classList.add('is-active');
    }

    newTrack.eqGains = [...track.eqGains];
    newTrack.fxState = JSON.parse(JSON.stringify(track.fxState));

    newTrack.trimStart = track.trimStart;
    newTrack.trimEnd = track.trimEnd;
    newTrack.fadeIn = track.fadeIn;
    newTrack.fadeOut = track.fadeOut;
    newTrack.mutes = track.mutes ? JSON.parse(JSON.stringify(track.mutes)) : [];

    aplicarFxStem(newTrack);
    aplicarEqStem(newTrack);
    aplicarEstadosMixerStems();
    renderTrimMutes(newTrack);

    const audio = document.getElementById('masterAudio');
    if (audio && !audio.paused) {
        newTrack.player.play().catch(() => {});
        sincronizarStemsComMaster(true);
    }
}

function removerStem(track) {
    if (!track) return;
    ...
    try { track.row.remove(); } catch (e) {}

    masterStemState.tracks = masterStemState.tracks.filter(t => t !== track);
...
function bindStemMasterSync() {
    ...
    const loopPlayhead = () => {
        if (!audio.paused) {
            ...
            // Metrônomo TICK
            const metronomeOn = document.getElementById('stemMetronomeBtn')?.classList.contains('is-on');
            if (metronomeOn) {
                const bpm = Number(document.getElementById('stemBpmInput')?.value || 120);
                const beatDuration = 60 / bpm;
...
async function renderizarStems(resultado) {
    ...
    if (!controls) {
        ...
        // Botão Efeitos Master
        const btnMasterFx = document.createElement('button');
        btnMasterFx.id = 'stemMasterFxBtn';
        btnMasterFx.className = 'presetBtn';
        btnMasterFx.type = 'button';
        btnMasterFx.innerHTML = '🎛️ Efeitos Master';
        btnMasterFx.title = 'Abrir rack de efeitos global (Master)';
        btnMasterFx.onclick = () => {
            if (masterStemState.masterTrack) {
                abrirModalFxStem(masterStemState.masterTrack);
            } else {
                alert('Mixer de áudio não inicializado.');
            }
        };
        controls.appendChild(btnMasterFx);

        // Seletor de Snap
        const snapContainer = document.createElement('div');
        snapContainer.style.display = 'flex';
        snapContainer.style.alignItems = 'center';
        snapContainer.style.gap = '6px';
        ...
        snapSelect.id = 'stemSnapSelect';
        snapSelect.className = 'formatSelect';
        snapSelect.style.padding = '4px 8px';
        snapSelect.style.borderRadius = '6px';
        snapSelect.style.height = 'auto';
        ...
        snapContainer.appendChild(snapSelect);
        controls.appendChild(snapContainer);

        // Botão de Metrônomo
        const btnMetronome = document.createElement('button');
        btnMetronome.id = 'stemMetronomeBtn';
        btnMetronome.className = 'presetBtn';
        btnMetronome.type = 'button';
        btnMetronome.innerHTML = '🔊 Metrônomo: OFF';
        btnMetronome.title = 'Alternar clique do metrônomo';
        btnMetronome.onclick = () => {
            btnMetronome.classList.toggle('is-on');
            if (btnMetronome.classList.contains('is-on')) {
                btnMetronome.innerHTML = '🔊 Metrônomo: ON';
                ensureStemAudioContext();
            } else {
                btnMetronome.innerHTML = '🔊 Metrônomo: OFF';
            }
        };
        controls.appendChild(btnMetronome);

        // Input de BPM
        const bpmContainer = document.createElement('div');
        bpmContainer.style.display = 'flex';
        bpmContainer.style.alignItems = 'center';
        bpmContainer.style.gap = '6px';

        const bpmLabel = document.createElement('span');
        bpmLabel.textContent = 'BPM:';
        bpmLabel.style.fontSize = '14px';
        bpmLabel.style.color = '#ccc';

        const bpmInput = document.createElement('input');
        bpmInput.id = 'stemBpmInput';
        bpmInput.type = 'number';
        bpmInput.min = '40';
        bpmInput.max = '240';
        bpmInput.value = '120';
        bpmInput.className = 'formatSelect';
        bpmInput.style.width = '70px';
        bpmInput.style.padding = '4px 8px';
        bpmInput.style.borderRadius = '6px';
        bpmInput.style.height = 'auto';
        ...
        controls.appendChild(bpmContainer);

        // Botão Adicionar Nova Faixa
        const btnAddTrack = document.createElement('button');
        btnAddTrack.id = 'stemAddTrackBtn';
        btnAddTrack.className = 'presetBtn';
        btnAddTrack.type = 'button';
        btnAddTrack.textContent = 'Adicionar Nova Faixa';
        btnAddTrack.title = 'Criar uma nova faixa de áudio vazia no projeto';
        btnAddTrack.onclick = async () => {
            const duracao = masterStemState.timelineDuration || 60;
            const blob = criarAudioSilencioso(duracao);
            const url = URL.createObjectURL(blob);
            const take = masterStemState.tracks.filter(t => t.row.dataset.title.startsWith('Nova Faixa')).length + 1;
            await adicionarFaixaStem({
                titulo: `Nova Faixa ${take}`,
                nome: `nova-faixa-${take}.wav`,
                url: url,
                isLocalRecording: true
            });
            setMasterStatus(`Nova faixa de áudio criada.`);
        };
        controls.appendChild(btnAddTrack);

        // Botão Importar Áudio
        const btnImportAudio = document.createElement('button');
        btnImportAudio.id = 'stemImportAudioBtn';
        btnImportAudio.className = 'presetBtn';
        btnImportAudio.type = 'button';
        btnImportAudio.textContent = 'Importar Áudio';
        btnImportAudio.title = 'Importar um arquivo de áudio (MP3, WAV, etc.) como nova faixa';
        btnImportAudio.onclick = () => {
            ...
            fileInput.onchange = async (e) => {
                ...
                setMasterStatus(`Áudio importado: ${file.name}`);
            };
            fileInput.click();
        };
        controls.appendChild(btnImportAudio);

...
def artista_albuns(slug):
    ...
    # 🔥 buscar artista
    c.execute(
        "SELECT id, nome FROM artistas WHERE slug=?",
        (slug,)
    )
    ...
    pasta_fotos_artista = os.path.join(
        "static",
        "fotos",
        "artista",
        nome_pasta
    ...
    if os.path.exists(mini_path):
        foto_url = (
            f"/static/fotos/artista/"
            f"{nome_pasta}/mini.jpg"
        )
    else:
        foto_url = (
    ...
    for aid, nome_album, ano, capa_url in albuns:
        ...
        if capa_url and str(capa_url).strip():

            cover_src = capa_url

        else:

            cover_src = buscar_capa_deezer(
                nome,
                nome_album
            )
